...
async def handle_b3_command(uid, chat_id, key, iv, region, chat_type, bot_uid):
    ...
    try:
        hud_json = '{"id":3099063260,"Name":"[B][C][I][FF0000] RXGAMERZ","Level":999,"MemberNum":"(999/999)","Apply":1,"Declaration":"\u0634\u064A\u0639\u0627\u0631","m_LimitLevel":100,"m_LimitRank":321,"m_LimitCSRank":321,"m_ClanBadgeID":10,"m_EntryType":1,"type":"clan"}'
...
async def oproom_and_invite(target_uid: int, key: bytes, iv: bytes, target_nickname: str = None):
    ...
    open_fields = {
        1: 2,
        2: {
            1: 1, 2: 15, 3: 5, 4: "[c][FF0000] RXGAMERZ", 5: "1", 6: 12, 7: 1, 8: 1, 9: 1,
...
async def revenge_then_offline(target_tc, key, iv, region, bot_self_uid):
    ...
    success = await run_ghost_task(target_tc, "RX", bot_self_uid, key, iv, region)
    if success:
...
#RX CONNECTION LOST
lx_burst_running = False
# =================== RXGAMERZ BOT CONSTANTS ===================
TAHMID = "RXGAMERZ_2026"
friends_list_running = False
friends_stop_flag = False
friends_current_index = 0
friends_total = 0
...
📱 Player will see:
"RXGAMERZFF  wants to join your team"
...
async def clan_message_loop():
    ...
    try:
        ...
        if _clan_msg_clan_id and _clan_msg_clan_id > 0 and _clan_msg_key and _clan_msg_iv:
            clan_msg = "Hello everyone this is RX Bot thanks for using me . Every day i need 500 glory or you will be kicked directly ."
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF] RXGAMERZ [00FF00]!!
"""
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF] RXGAMERZ [FFFF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @RXGAMERZ965\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="RX", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="RX", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg =  f"""[B][C][FFFF00] এই শা🤫লা বো👽কা👽চু👽দা তুই RX কে নুব বলছিস তোর মাকে কন👽ডম চু👽দি 
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="RX", title_id=title_id)
                    ...
                    if data_hex.startswith("0500"):
                        ...
                        if packet_json.get("1") == 25:
                            ...
                            if new_friend_uid:
                                ...
                                friend_msg = "hi this using RXGAMERZ bot thanks for using me"
...
[b][ffdd00][c]◉━━━━━━━━━━━━━━◉
[FFDDAA]     স্বা🥰গত🥰ম RXGAMERZ এ💀র BOT এ
[b][ffdd00] 👽 BOT OWNER : [FFFFFF] RXGAMERZ 
[DDDDDD]👽 TIKTOK USER👽NA👽ME : [55DDAA]@RXGAMERZ965
[FF4400]👽 YouTube: [FFFFFF]@RXGAMERZ965
...
[b][c][A52A2A]TELEGRAM CONTACT @RXGAMERZ965
...
[B][00FFFF]Example: /ghost 4337272 RX
"""
...
#GALI SPAM MESSAGE 
    ...
    "RXGAMERZ", "RXGAMERZ", "TAHMID", "RXGAMERZ",
                        ...
                        if original_msg and "StickerStr" in original_msg:
                            ...
                            if NUMBER_EMOTES:
                                ...
                                try:
                                    ...
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)
                                    # Also send a message
                                    sticker_reply = "[B][C][FFFF00] RXGAMERZ STYLE TCP"
